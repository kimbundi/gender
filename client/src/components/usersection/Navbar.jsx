...
const Navbar = () => {
  ...
  const { openSignIn, signOut } = useClerk(); // Get signOut from useClerk
  const { user } = useUser();
  ...
  return (
    <div
      className={`flex items-center justify-between px-4 sm:px-10 md:px-14 lg:px-36 border-b border-gray-500 py-4 ${
        isGenderlistpage ? 'bg-white' : 'bg-cyan-100/70'
      }`}
    >
      <img
        onClick={() => navigate('/')}
        src={assets.logo}
        alt="logo"
        className="w-20 h-20 rounded-full lg:w-20 cursor-pointer"
      />
      <div className="hidden md:flex items-center gap-5 text-gray-500">
        <div className="flex items-center gap-5">
          {user && (
            <>
              <Link to={'/reportid'}>Report ID</Link>
      ...
      <div className="md:hidden flex items-center gap-2 sm:gap-5 text-gray-500">
        ...
        {user ? (
          ...
          <button onClick={() => openSignIn()}>
            <img src={assets.usericon} alt="" className="w-7 h-6" />
          </button>
        )}
      </div>
    </div>
